...
export function jNMI(array1, array2) {
  ...
  function entropy(array) {
    ...
    Object.keys(aux).forEach(function (value, key) {
      entropy =
        entropy -
        (aux[value] / array.length) * Math.log2(aux[value] / array.length);
    });
  ...
  function mutualInformation(array1, array2) {
    ...
    Object.keys(clusters_array2).forEach(function (value1, key1) {
      // For each cluster.

      array1_array2 = [];

      for (let j = 0; j < clusters_array2[value1].length; j++) {
        // In array1

        array1_array2.push(array1[clusters_array2[value1][j]]);
      }

      entropy_array[value1] =
        (entropy(array1_array2) * array1_array2.length) / array1.length;
    });
  ...
  if (entropy(array1) + entropy(array2) === 0) {
    ...
    return (
      (2 * mutualInformation(array1, array2)) /
      (entropy(array1) + entropy(array2))
    );
  }
}
